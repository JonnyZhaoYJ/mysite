...
class RegForm(forms.Form):
    ...
    def __init__(self,*args,**kwargs):
        if 'request' in kwargs:
            self.request = kwargs.pop('request')  # pop 剔除
        super(RegForm,self).__init__(*args,**kwargs)

    # 判断验证码
    def clean(self):
        code = self.request.session.get('register_code','')   # 系统生成的
        verification_code = self.cleaned_data.get('verification_code','')   # 用户填的
...
class ChangeNicknameForm(forms.Form):
    ...
    def __init__(self,*args,**kwargs):
        if 'user' in kwargs:
            self.user = kwargs.pop('user')  # pop 剔除
...
class BindEmailForm(forms.Form):
    ...
    def clean(self):
        ...
        # 判断验证码
        code = self.request.session.get('bind_email_code','')   # 系统生成的
        verification_code = self.cleaned_data.get('verification_code','')   # 用户填的
        if not (code != '' and code == verification_code):
            raise forms.ValidationError('验证码不正确')
...
class ForgotPasswordForm(forms.Form):
    ...
    def clean_verification_code(self):
        ...
        # 判断验证码
        code = self.request.session.get('forgot_password_code','')   # 系统生成的
        if not (code != '' and code == verification_code):
            raise forms.ValidationError('验证码不正确')
